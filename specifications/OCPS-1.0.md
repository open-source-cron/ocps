...
1.  **List Separation (`,`)**: The comma is the top-level separator. It breaks a field into a list of independent sub-expressions. The final set of values for the field is the union (logical `OR`) of the results of each sub-expression. A parser MUST parse and validate every sub-expression in the list, even if an earlier sub-expression already yields the full possible range for the field.
2.  **Range (`-`) or Wildcard (`*`)**: Within each sub-expression, a range (`10-20`) or a wildcard (`*`) defines the initial set of numbers. A wildcard is equivalent to the full possible range for that field (e.g., `0-59` for minutes). Accordingly, an unrestricted wildcard MAY appear as one entry within a list (e.g., `*,1-30`), and if all other entries are valid the resulting set for the field is still the full possible range.
...
#### Out-of-Range Values
A parser MUST reject a pattern if any numeric value falls outside the allowed range for its field (e.g., `60` in the minute field, `32` in the day-of-month field). This MUST be treated as a parsing error. This requirement applies to every list sub-expression independently; for example, `*,*,61` in the minute field is a parsing error even though `*` already covers the full range.
...
* A step operator not preceded by `*` or a full range `A-B` (e.g., `/30`, `0/15`, or `10/10`). The `/` character MUST be preceded by either `*` or a range in the form `A-B` where both A and B are specified (e.g., `*/30` or `10-50/10` are valid, but `/30`, `0/15`, or `10/10` are not).
